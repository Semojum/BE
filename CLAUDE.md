...
| JOB4012 | 400 | 다운로드할 변환 결과 없음 (FAILED 잡 등) |
...
### Job
- `POST /api/jobs` — Job 생성 (multipart, `mode` + **`insertPageNumber`**(선택, 기본 false) + **`footerText`**(선택, 꼬리말 묵자 최대 200자 — `jobs.footer_text`(V15) 저장, 다운로드 때 점역))
