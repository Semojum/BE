...
- **폴더 정렬 기준은 `folders.last_modified_at`**(V12) — 생성일이 아니다. 갱신 시점: ① 직속 항목의 추가·삭제·이름변경(윈도우 탐색기와 동일) ② **직속 파일의 내용 편집**(탐색기와 다른 점 — 점역사의 주된 활동이라 반영하지 않으면 최신순 정렬이 무의미해짐). **상위로 전파하지 않는다**. 갱신은 `FolderTouch` 한곳에 모여 있으니 폴더 안 항목을 바꾸는 코드를 추가하면 여기도 호출할 것
