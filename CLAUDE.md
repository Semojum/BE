...
- **각주·미주 추출**: 각주(`ControlFootnote`)는 원문 레이아웃과 같이 **그 페이지 끝에** `[각주]` 마커와 함께, 미주(`ControlEndnote`)는 **문서 끝에** `[미주]`로 모음. 이전에는 표만 처리해 각주가 통째로 누락됐음(KSCI 논문양식의 저자 연락처·투고일·Copyright 5줄 소실 확인). **머리말·꼬리말은 페이지마다 반복되는 판면 요소라 본문으로 추출하지 않음**
