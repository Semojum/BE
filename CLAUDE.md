...
- **폴더 목록 API 두 종류**: `GET /api/folders?parentId=` (한 단계 자식 — 폴더 내부 화면 S2·첫 화면. 생략 시 최상위) / `GET /api/folders/tree` (전체 중첩 트리 — 이동 모달처럼 구조 전체가 필요한 화면). S2는 이 API + `GET /api/users/jobs?folderId=`로 폴더와 파일을 각각 받는다
